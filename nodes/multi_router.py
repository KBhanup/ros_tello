...
from ros_tellopy.msg import Vector3Array, Vector4
from tello.tello import Tello
from tello.tello import TelloDataStream


class Individual:
    def __init__(self, name, ip, local_port):
        ...
        self.rc_control = rospy.Subscriber(self.name + '/rc_control', Vector4, self.handle_rc_control)
    ...
    def handle_rc_control(self, msg):
        print(self.name)
        self.drone.rc_control(int(msg.x*100), int(msg.y*100), int(msg.z*100), int(msg.w*100))
...
class Multi:
    def handle_state(self, data, ip):
        self.drones[ip[0]].handle_state(data)
    ...
    def __init__(self):
        ...
        self.drones = {}
        local_port = 8891
        for tello in tello_info:
            name = tello['name']
            ip = tello['ip']
            port = local_port
            self.drones[ip] = Individual(name, ip, port)
            local_port += 1

        self.data_streamer = TelloDataStream(self.handle_state)

        while not rospy.is_shutdown():
            for drone in self.drones.values():
                drone.drone.command_noresponse()
